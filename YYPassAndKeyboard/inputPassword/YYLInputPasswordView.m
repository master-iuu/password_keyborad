...
#define yyl_BottomSafeAreaHeight (isiPhoneX ? 34.0 : 0)
...
- (void)showInputPasswordViewWithTranamt:(NSString *)tranamt{
    ...
    [self.payPassView settingTranamt:tranamt];
...
- (void)addSubviews{
    ...
    [self prohibitSideslipBack];
}
/**
 禁用侧滑返回
 */
- (void)prohibitSideslipBack{
...
- (YYLCustomKeyboardView *)keyboardView {
    if (_keyboardView == nil) {
        _keyboardView = [[YYLCustomKeyboardView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-200-yyl_BottomSafeAreaHeight, self.bounds.size.width, 200)];
    }
